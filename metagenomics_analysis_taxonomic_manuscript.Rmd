...
tmp <- as.matrix(taxa_clr[,3:ncol(taxa_clr)])
rownames(tmp) <- taxa_clr$Taxon
taxa_clr.tmp <- t(logratio.transfo(t(tmp), logratio = "CLR", offset = 0.001))
...
rownames(tmp) <- taxa_count_clr$Taxon
taxa_count_clr.tmp <- t(logratio.transfo(t(tmp), logratio = "CLR", offset = 0.001))
...
# Remove low-abundance taxa
taxa_count_rm0 <- taxa_count[-which(taxa_count$Taxon %in% zero_rm_taxa),]
taxa_count_rm0_clr <- taxa_count_rm0

tmp <- as.matrix(taxa_count_rm0_clr[,3:ncol(taxa_count_rm0_clr)])
rownames(tmp) <- taxa_count_rm0_clr$Taxon
taxa_count_rm0_clr.tmp <- t(logratio.transfo(t(tmp), logratio = "CLR", offset = 0.001))
class(taxa_count_rm0_clr.tmp) <- "matrix"
taxa_count_rm0_clr <- data.frame(Taxon = taxa_count_rm0$Taxon, GTDB.taxonomy = taxa_count_rm0$GTDB.taxonomy, taxa_count_rm0_clr.tmp)
...
summary(lm(shannon_food ~ shannon + age + proforma_sex + diet_PC1 + diet_PC2 + diet_PC3 + as.numeric(bristol_stool_chart_regroup), data = diversity_asd.df))

diversity_non.df <- diversity.df %>% filter(participant_type %in% c("SIB", "UNR"))
summary(lm(shannon_food ~ shannon, data = diversity_non.df))
summary(lm(shannon_food ~ shannon + age + proforma_sex + diet_PC1 + diet_PC2 + diet_PC3 + as.numeric(bristol_stool_chart_regroup), data = diversity_non.df))
...
ggstatsplot::combine_plots(
  ...
  title = "Diet ~ Sensory score", stats.label.color = cols_ssp,
  ylab = NULL, stats.label.args = c(nudge_y = 0.3), exclude.intercept = TRUE) + theme(text = element_text(size = 15)),
  # # X-trait PGS
